...
public static class EntraAuthentication {
    ...
    public static string Key(string tenant, string objectId) => $"{Guid.Parse(tenant):D}:{Guid.Parse(objectId):D}";
    ...
    public static ApplicationUser ResolveUser(IObjectSpace objectSpace, ClaimsPrincipal principal) {
        ...
        var login = objectSpace.FirstOrDefault<ApplicationUserLoginInfo>(x =>
            x.LoginProviderName == Scheme && x.ProviderUserKey == key);
