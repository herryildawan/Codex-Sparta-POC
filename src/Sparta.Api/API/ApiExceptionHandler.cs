...
public sealed class ApiExceptionHandler : IExceptionHandler {
    public async ValueTask<bool> TryHandleAsync(HttpContext context, Exception exception, CancellationToken cancellationToken) {
        ...
        await Results.Problem(statusCode: status, detail: detail, title: status switch {
            400 => "Business validation failed.", 403 => "The operation is not permitted.", _ => "The record changed. Reload and retry."
        }, extensions: new Dictionary<string, object?> { ["traceId"] = System.Diagnostics.Activity.Current?.TraceId.ToString() })
