...
public class InventoryCommandsController(IObjectSpaceFactory factory, ISecurityStrategyBase security) : ControllerBase {
    ...
    public IActionResult Update(int id, WarehouseUpdate request) {
        ...
        os.CommitChanges(); return NoContent();
    }
...
public record WarehouseUpdate([Required, MaxLength(32)] string Code, [Required, MaxLength(200)] string Name,
    bool IsActive, [Required] string RowVersion);
public record MovementPost([Range(1, int.MaxValue)] int ProductId, [Range(1, int.MaxValue)] int WarehouseId,
    decimal QuantityDelta, DateTimeOffset OccurredAt, [Required, MaxLength(100)] string Reference);
